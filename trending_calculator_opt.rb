...
class IndexLine
  ...
  def sim?(o, diff_delta, line_delta)
    line_diff = (o.get_point(@index) - @base).round(3).abs
    diff_diff = (@diff - o.diff).round(3).abs
    return (((line_diff == 0) and (diff_diff < diff_delta)) or
      ((line_diff < line_delta) and (diff_diff == 0)))
  end
